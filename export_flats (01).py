...
Если пути к моделям не указаны, скрипт ищет *.uploads / *.ifczip
...
def main():
    ...
    # если не указаны – ищем *.uploads *.ifczip рядом со скриптом
    if not paths:
        script_dir = os.path.dirname(os.path.realpath(__file__)) or "."
        paths = sorted(
            glob.glob(os.path.join(script_dir, "*.uploads")) +
            glob.glob(os.path.join(script_dir, "*.ifczip"))
        )
        if not paths:
            print("Файлы *.uploads / *.ifczip не найдены.")
            return
